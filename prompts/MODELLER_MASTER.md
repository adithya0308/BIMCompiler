...
## bim-ootb `origin/main` @ `3e8c6be2` by grep/sqlite, plus 4 witnesses actually RUN. Read this table,
...
| 28 | verified-open (contradiction) | verified-open — **sharpened, and a shipped comment is FALSE** | Re-measured the shipped `modeller/SampleCastle_ARC.db`: **293** rows non-zero, and every one of them is `rotation_y = ±1.5707963267949` **exactly**, with `rotation_x = 0` and `rotation_z = 0` on all 293. So these are not arbitrary "tilts" — they are exact ±90° about Y. Classes: IfcCovering 125 · IfcWindow 80 · IfcWall 32 · IfcDoor 28 · IfcWallStandardCase 21 · IfcRailing 7. ⚠ **`arc_editable.js:232`'s own comment asserts "Duplex/SampleHouse/SampleCastle: 0 non-zero rotation_x/rotation_y rows" — that is false against the DB the roster actually serves.** Fix the comment or the claim before trusting anything downstream of it. |
...
|---|---|---|---|---|
| 35 | O13 | **The LIVE guide contradicts shipped behaviour.** The published ModellerGuide says *"Hosted doors and windows **ride**, never distort"* (1 hit live, and **0** hits of "ANCHORED by default") — but **#1706** (2026-09-11) made openings **anchor by default with ride opt-in**, the exact reverse. Master already fixed the text in `2c232661c` ("fix stale Grid-Stretch anchor/ride claim, add 6 missing icons + Save section"); it was **never deployed**. `docs/ModellerGuide.md` master blob `7ce09190` ≠ deployed blob `eed2afdf`; gh-pages was deployed from `58e7f344a` on **2026-09-04**, and 2 doc commits have landed since | run `mkdocs gh-deploy` (`safe_gh_deploy.sh`), then re-curl and assert "ANCHORED by default" is present and "ride, never distort" is gone | verified-open — **the only row here that is actively misleading a real user today** |
